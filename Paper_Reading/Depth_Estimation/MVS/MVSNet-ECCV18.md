...
In order to achieve this, we have to resort to the homography, a.k.a, planar transformation, to establish the bridge between depth and correspondence. A homography is a $3 \times 3$ matrix with $DoF = 8$ and is only good to depict transformation of the major planar region, leaving other planar layers within the reference image poorly fitted. Pure rotation is its special case which regard the whole image as a plane. However, with depth varying, there would be multiple homography matrices projecting each pixel at different scale to locate which correspondence in source image it should associate to. Note that here backward-projection (from $\mathbf{I}_i$ to $\mathbf{I}_j$) is adopted to ensure matching completeness. Specifically,
$$
\begin{equation}
x_j = \mathbf{H}_{ji}(d) \cdot x_i,
\end{equation}
$$
$$
\begin{equation}
\mathbf{H}_{ji}(d) = \mathbf{K}_j \cdot \mathbf{R}_j \cdot (\mathbf{I} - \frac{(\mathbf{R}^T_j \cdot \mathbf{t}_j - \mathbf{R}^T_i \cdot \mathbf{t}_i) \cdot n^T_i \cdot \mathbf{R}_i}{d}) \cdot \mathbf{R}^T_i \cdot \mathbf{K}^{-1}_i.
\end{equation}
$$
...
$$
\begin{equation}
X_j = \mathbf{R}_{ji} \cdot X_i + \mathbf{t}_{ji}.
\end{equation}
$$
...
$$
\begin{equation}
-\frac{n^T_i \cdot X_i}{d} = 1,
\end{equation}
$$
$$
\begin{equation}
\mathbf{t} = \mathbf{t} \cdot (-\frac{n^T \cdot X}{d}).
\end{equation}
$$
...
$$
\begin{equation}
X_j = \mathbf{R}_j \cdot \mathbf{R}^T_i \cdot X_i - (\mathbf{t}_j - \mathbf{R}_j \cdot \mathbf{R}^T_i \cdot \mathbf{t}_i) \cdot \frac{n^T_i \cdot X_i}{d}
\end{equation}
$$
$$
\begin{equation}
X_j = \mathbf{R}_j (\mathbf{I} - (\mathbf{R}^T_j \cdot \mathbf{t}_j - \mathbf{R}^T_i \cdot \mathbf{t}_i) \cdot \frac{n^T_i}{d} \cdot \mathbf{R}_i) \cdot \mathbf{R}^T_i \cdot X_i.
\end{equation}
$$
...
# Cost Volume
Since the homography process helps to establish correspondences across different views, the truly matched pixels should have similar features subject to $d$. So by aggregating all feature volume into one, the final formed volume $\mathbf{C} \in [D, 32, 128, 160]$, a.k.a., cost volume, measures the variance of feature similarity along the first dimension. 
$$
\begin{equation}
\mathbf{C} = \frac{\sum \limits^{N}_{i=1} (\mathbf{V}_i - \bar{\mathbf{V}})^2}{N}.
\end{equation}
$$
...
$$
\begin{equation}
\hat d = \sum \limits^{dmax}_{d = dmin} d \times \mathbf{P}(d).
\end{equation}
$$
...
$$
\begin{equation}
Loss = \sum \limits_{p \in \mathbf{P}_{valid}} \| \mathbf{d}(p) - \hat{\mathbf{d}}(p) \|_1 + \lambda \| \mathbf{d}(p) - \widetilde{\mathbf{d}}(p) \|_1,
\end{equation}
$$
