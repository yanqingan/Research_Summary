...
In inference, in contrast to the other steps, the last transmit directly utilizes the estimated mean $\frac{1}{\sqrt{\alpha_1}} x_1 - \frac{1 - \alpha_1}{\sqrt{1 - \bar{\alpha}_1} \sqrt{\alpha_1}} \epsilon_{\theta}(x_1, 1)$ as final output $\hat{x}_0$ rather than drawing a sample from the distribution. This is because: (1) It is regularized in the reconstruction term; (2) It can improve the determinism as the last step has very low variance.
